    ...
    class ViewLayoutsBaseHtmlHeadHook < Redmine::Hook::ViewListener
      def view_layouts_base_html_head(context={})
        if context[:controller] && (  context[:controller].is_a?(IssuesController) ||
          ...
          return stylesheet_link_tag("jquery.fancybox-3.5.7.min.css", :plugin => "redmine_x_lightbox2", :media => "screen") +
            stylesheet_link_tag("lightbox.css", :plugin => "redmine_x_lightbox2", :media => "screen") +
            javascript_include_tag('jquery.fancybox-3.5.7.min.js', :plugin => 'redmine_x_lightbox2') +
            javascript_include_tag('lightbox.js', :plugin => 'redmine_x_lightbox2')
        else
          return ''
        end
      end
