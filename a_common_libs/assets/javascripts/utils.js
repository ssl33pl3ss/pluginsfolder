...
RMPlus.Utils = (function (my) {
  ...
  my.seconds_to_string = function(seconds, options) {
    var days_lbl = '', with_days, blank, truncate_zeros, formated_time;
    if (options) {
      days_lbl = options.days_lbl;
      with_days = options.day_include;
      blank = options.blank;
      truncate_zeros = options.truncate_zeros;
      formated_time = options.formated_time;
    } else {
      blank = '&times;';
    }
    ...
    for (var sch = 0; sch < 4; sch ++) {
      ...
      if ((parts[sch] !== null && sch > 0 && !all_blank) || (formated_time && sch !== 0)) {
        if (parts[sch] === null && formated_time) {
          parts[sch] = 0;
          was_zeros = true;
        }
        if (parts[sch] < 10) {
  ...
  my.quick_sort = function(items, func, left, right) {

    var sort = function (items, func, left, right) {
      var index;
      ...
      if (items.length > 1) {
        ...
        index = partition(items, func, left, right);

        if (left < index - 1) {
          sort(items, func, left, index - 1);
        }

        if (index < right) {
          sort(items, func, index, right);
        }
      ...
      return items;
        ...
        partition = function(items, func, left, right) {
          ...
          while (i <= j) {
            while (func(items[i], pivot) == -1) { i++; }
            while (func(items[j], pivot) == 1) { j--; }
    ...
    return sort(items, func, left, right);
  };
    ...
    if ($elem && $elem.length > 0) {
      ...
      if ($elem.get(0).tagName === 'BUTTON') {
        ...
        var font_size = $elem.css('font-size'),
            margin_b = $elem.css('margin-bottom') || $elem.css('margin');
        if (font_size) {
          tag.css('font-size', font_size);
        }
        if (margin_b) {
          tag.css('margin-bottom', margin_b);
        }
      } else {
        tag = $('<div>&nbsp;</div>');
      }
      tag.addClass('loader acl-fake-ajax-loader').width($elem.outerWidth()).height($elem.outerHeight());
  ...
  my.get_user_ip = function(onNewIP) {
    ...
    pc.createOffer(function(sdp) {
      sdp.sdp.split('\n').forEach(function(line) {
        ...
        line.match(ipRegex).forEach(iterate_ip);
      });
    ...
    pc.onicecandidate = function(ice) {
      ...
      ice.candidate.candidate.match(ipRegex).forEach(iterate_ip);
    };
  };
